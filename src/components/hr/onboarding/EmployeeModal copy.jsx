...
const EmployeeModal = ({ isOpen, onClose, employeeId = null }) => {
  ...
  // Add new state for currently editing skill
...
//   const fetchSkillsData = async (empId = savedEmployeeId) => {
//   if (!empId) {
...
//   try {
//     console.log('Fetching skills data for employee:', empId);
//     const response = await EmployeeSkillsAPI.getByEmployeeId(empId);

//     console.log('Skills API Response:', response);

//     if (response.success && response.result && Array.isArray(response.result)) {
//       // Map the array of skills with correct property names
...
//       // Set the first skill as editing skill if exists
...
const handleInputChange = (section, field, value) => {
  // if (section === 'skills') {
  //   // Handle skills separately using editingSkill state
...
//             console.log('Skills Update Data:', skillsData);
//           } else {
//             // ADD MODE
//             skillsData = {
//               employee_id: savedEmployeeId,
...
//             console.log('Skills Add Data:', skillsData);
//           }

//           const response = isUpdate
//             ? await EmployeeSkillsAPI.update(skillsData)
//             : await EmployeeSkillsAPI.add(skillsData);

//           if (response.success) {
//             // Update the skills array in formData
//             if (isUpdate) {
//               // Update existing skill in array
//               setFormData(prev => ({
//                 ...prev,
...
//               }));
//             } else {
//               // Add new skill to array with returned ID
//               const newSkill = {
//                 ...editingSkill,
...
//               setCurrentSkillIndex(formData.skills.length);
//             }
            
//             alert(`Skill ${isUpdate ? 'updated' : 'added'} successfully!`);
//             return true;
//           } else {
//             showErrorWithRetry(response.message || 'Failed to save skill details');
//             return false;
//           }
//         } catch (error) {
...
//     setEditingSkill(formData.skills[prevIndex]);
//     setErrors({});
//   }
// };

// const handleNextSkill = () => {
//   if (currentSkillIndex < formData.skills.length - 1) {
//     const nextIndex = currentSkillIndex + 1;
//     setCurrentSkillIndex(nextIndex);
//     setEditingSkill(formData.skills[nextIndex]);
//     setErrors({});
//   }
// };
...
//       const updatedSkills = formData.skills.filter((_, idx) => idx !== currentSkillIndex);
//       setFormData(prev => ({
//         ...prev,
      ...
      <div className="employee-modal">
        ...
        <div className="employee-modal-body">
          <div className="sidebar">
            {tabs.map(tab => {
              const isAccessible =
                ...
                // (tab.id === 'skills' && savedEmployeeId && formData.passport.id);

              const isCompleted =
                (tab.id === 'personal' && savedEmployeeId) ||
                (tab.id === 'passport' && formData.passport.id);
                // (tab.id === 'skills' && formData.skills.id);
          ...
          <div className="content-area">
            ...
            {/* {currentTab === 'skills' && (
              <div className="tab-content p-6 bg-white rounded-2xl shadow-md">
                <h3 className="text-2xl font-semibold text-gray-800 mb-4 border-b pb-2">Skills Details</h3>

                
                <div className="skills-navigation flex flex-col md:flex-row justify-between items-center mb-6 gap-3">
                  <div className="skills-counter text-gray-700 font-medium">
                    Skill {currentSkillIndex + 1} of {Math.max(formData.skills.length, currentSkillIndex + 1)}
                  </div>
                  <div className="skills-buttons flex flex-wrap gap-3">
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      disabled={currentSkillIndex >= formData.skills.length - 1}
                      className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg border border-gray-300 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition"
                    >
                      Next →
                    </button>
                ...
                <div className="form-group mb-4">
                  <label className="block text-gray-700 font-medium mb-1">Skill Area *</label>
                  <input
                    type="text"
                    value={editingSkill.skill_area}
                    onChange={(e) => handleInputChange('skills', 'skill_area', e.target.value)}
                    className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      errors.skill_area ? 'border-red-500' : 'border-gray-300'
                    }`}
                  />
                ...
                <div className="form-group mb-4">
                  <label className="block text-gray-700 font-medium mb-1">Skills Acquired *</label>
                  <textarea
                    value={editingSkill.skills_acquired}
                    onChange={(e) => handleInputChange('skills', 'skills_acquired', e.target.value)}
                    className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      errors.skills_acquired ? 'border-red-500' : 'border-gray-300'
                    }`}
                    rows="3"
                  />
                ...
                <div className="form-group mb-4">
                  <label className="block text-gray-700 font-medium mb-1">Certification Grade *</label>
                  <input
                    type="text"
                    value={editingSkill.certification_grade}
                    onChange={(e) => handleInputChange('skills', 'certification_grade', e.target.value)}
                    className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      errors.certification_grade ? 'border-red-500' : 'border-gray-300'
                    }`}
                  />
                ...
                <div className="form-group mb-6">
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleInputChange('skills', 'project_summary', e.target.value)}
                    className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      errors.project_summary ? 'border-red-500' : 'border-gray-300'
                    }`}
                    rows="4"
                  />
                  {errors.project_summary && <span className="text-red-500 text-sm">{errors.project_summary}</span>}
                </div>
                ...
                <div className="form-actions text-right">
                  <button
                    ...
                    disabled={isLoading}
                    className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                  >
